...
```{r}
LDNA1 <- read.table("LD/LD_na50_ddgbs_randomSW_topnotop_results_out_new")
LDEU1 <- read.table("LD/LD_eu50_ddgbs_randomSW_topnotop_results_out_new")
LDAU1 <- read.table("LD/LD_au50_ddgbs_randomSW_topnotop_results_out_new")
...
topSNP_na_2m_rand = read.table("LD/topSNP_na_2m50_randSW", header=T)
...
notopSNP_na_rand=  read.table("LD/notopSNP_na_2m50_randSW", header=T)
...
topSNP_eu_2m_rand = read.table("LD/topSNP_eu_2m50_randSW", header=T)
...
notopSNP_eu_rand=  read.table("LD/notopSNP_eu_2m50_randSW", header=T)
...
topSNP_au_2m_rand = read.table("LD/topSNP_au_2m50_randSW", header=T)
...
notopSNP_au_rand=  read.table("LD/notopSNP_au_2m50_randSW", header=T)
...
interactionMeans(mod, custom=contr)
...
Heatmaps
```{r}
G <- graph.data.frame(LDNA1,directed=FALSE)
A <- as_adjacency_matrix(G,names=TRUE,sparse=FALSE,attr = "V3",type='lower')
A=(A)^2
a<- hclust(dist((A)), method="ward.D2")
b<-hclust(dist(t(A)), method="ward.D2")

pdf("_Graph/LD_na_all.pdf")
heatmap3(A, Rowv=as.dendrogram(a), Colv = "Rowv", balanceColor = T, scale="none"       ) 
dev.off()



G <- graph.data.frame(top_na,directed=FALSE)
A <- as_adjacency_matrix(G,names=TRUE,sparse=FALSE,attr = "Corr",type='lower')
A=(A)^2
a<- hclust(dist((A)), method="ward.D2")
b<-hclust(dist(t(A)), method="ward.D2")

pdf("_Graph/LDtop_na.pdf")
heatmap3(A, Rowv=as.dendrogram(a), Colv = "Rowv", balanceColor = T, scale="none"       ) 
dev.off()

G <- graph.data.frame(top_eu,directed=FALSE)
A <- as_adjacency_matrix(G,names=TRUE,sparse=FALSE,attr = "Corr",type='lower')
A=(A)^2
a<- hclust(dist((A)), method="ward.D2")

pdf("_Graph/LDtop_eu.pdf")
heatmap3(A, Rowv=as.dendrogram(a), Colv = "Rowv", balanceColor = T, scale="none"       ) 
dev.off()



G <- graph.data.frame(top_au,directed=FALSE)
A <- as_adjacency_matrix(G,names=TRUE,sparse=FALSE,attr = "Corr",type='lower')
A=(A)^2
a<- hclust(dist((A)), method="ward.D2")

pdf("_Graph/LDtop_au.pdf")
heatmap3(A, Rowv=as.dendrogram(a), Colv = "Rowv", balanceColor = T, scale="none"       ) 
dev.off()
