...
*Binning SNPs*
```{r}
bf_naeu_lat$myrank_na <- rank(bf_naeu_lat$Latitude_all.x)/length(bf_naeu_lat$Latitude_all.x)
bf_naeu_lat$myrank_eu <- rank(bf_naeu_lat$Latitude_all.y)/length(bf_naeu_lat$Latitude_all.y)
bf_naau_lat$myrank_na <- rank(bf_naau_lat$Latitude_all.x)/length(bf_naau_lat$Latitude_all.x)
bf_naau_lat$myrank_au <- rank(bf_naau_lat$Latitude_all.y)/length(bf_naau_lat$Latitude_all.y)
bf_euau_lat$myrank_eu <- rank(bf_euau_lat$Latitude_all.x)/length(bf_euau_lat$Latitude_all.x)
bf_euau_lat$myrank_au <- rank(bf_euau_lat$Latitude_all.y)/length(bf_euau_lat$Latitude_all.y)

bf_naeu_lat$bin_na5 <- as.numeric(cut(bf_naeu_lat$myrank_na, 20, labels=1:20))
bf_naeu_lat$bin_eu5 <- as.numeric(cut(bf_naeu_lat$myrank_eu, 20, labels=1:20))

bf_naau_lat$bin_na5 <- as.numeric(cut(bf_naau_lat$myrank_na, 20, labels=1:20))
bf_naau_lat$bin_au5 <- as.numeric(cut(bf_naau_lat$myrank_au, 20, labels=1:20))

bf_euau_lat$bin_eu5 <- as.numeric(cut(bf_euau_lat$myrank_eu, 20, labels=1:20))
bf_euau_lat$bin_au5 <- as.numeric(cut(bf_euau_lat$myrank_au, 20, labels=1:20))
```
...
for(i in 1:nrow(bf_naeu_lat)){
  if(bf_naeu_lat$bin_na5[i] == bf_naeu_lat$bin_eu5[i]){
    bincount[i,] <- c(bf_naeu_lat$bin_na5[i], bf_naeu_lat$windID[i])
    }
}
...
bincount <- data.frame(bin = numeric(), scaf_pos = numeric())
for(i in 1:nrow(bf_naau_lat)){
  if(bf_naau_lat$bin_na5[i] == bf_naau_lat$bin_au5[i]){
    bincount[i,] <- c(bf_naau_lat$bin_na5[i], bf_naau_lat$windID[i])
    }
}
bincount$bin <- as.numeric(bincount$bin)
...
bincount <- data.frame(bin = numeric(), scaf_pos = numeric())
for(i in 1:nrow(bf_euau_lat)){
  if(bf_euau_lat$bin_eu5[i] == bf_euau_lat$bin_au5[i]){
    bincount[i,] <- c(bf_euau_lat$bin_eu5[i], bf_euau_lat$windID[i])
    }
}
bincount$bin <- as.numeric(bincount$bin)
...
pdf("~/Documents/Monash/PhD/Analyses/Data/_Graph/binall_bfsnp_lat.pdf",height=4,width=4)
  ggplot()+
...
pdf("~/Documents/Monash/PhD/Analyses/Data/_Graph/binall_bfwind_lat.pdf",height=4,width=4)
  ggplot()+
  geom_bar(data = obsexp_all, aes(percent, Freq, fill = ranges),stat = 'identity',position="dodge") +
